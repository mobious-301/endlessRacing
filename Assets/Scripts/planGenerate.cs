...
public class planGenerate : MonoBehaviour
{
    ...
    void creatShape(ref Vector3[] vertices,ref Vector2[] uvs,ref int[] triangle){
        ...
        for(int z=0;z<zCount;z++){
        	for(int x=0;x<xCount;x++){
                ...
                vertices[index]=new Vector3(x,0,z*scale*Mathf.PI);
				//平面模式检查模型错误
				...
				//两块平面首尾相接，要保证末尾x和y的偏移值都对接上
				px=vertices[index].x/(xCount-1);//x的值为0-1
				px= vertices[index].x/(xCount-1)*(float)Math.Pow(2,8)%1;//x的值为0-1 的循环,且为8/2次
				px= vertices[index].x/(xCount-1);
				Debug.Log(px);
                float py=vertices[index].z*perlinScale+offset;
				py=vertices[index].z/Mathf.PI;
				// py=vertices[index].z/Mathf.PI*(float)Math.Pow(2,-8);
				// Debug.Log(vertices[index].z/Mathf.PI);
                //中心线 顶点延归一化法线做偏移
                Vector3 center=new Vector3(0,0,vertices[index].z);
                // vertices[index]+=(center-vertices[index]).normalized*Mathf.PerlinNoise(0,py)*waveHeight;

				center=new Vector3(0,1,0);
				vertices[index]+=(center).normalized*Mathf.PerlinNoise(px,py)*waveHeight;
				// Debug.Log(((center).normalized*Mathf.PerlinNoise(px,py)*waveHeight).y);
                index++;
            }
        }
		// Debug.Log(loga);
        //三角形数组  一个矩形2个三角形 每个三角形三个顶点
        triangle=new int[(xCount-1)*(zCount-1)*6];
        //创建二位数组存储顶点方便调用
